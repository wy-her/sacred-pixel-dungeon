...
public class LineBuilder extends RegularBuilder {
	...
	public ArrayList<Room> build(ArrayList<Room> rooms) {
		...
		float[] pathTunnels = java.util.Arrays.copyOf(pathTunnelChances, pathTunnelChances.length);
		for (int i = 1; i < mainPathRooms.size(); i++){
			Room r = mainPathRooms.get(i);

			int tunnels = Random.chances(pathTunnels);
			if (tunnels == -1){
				pathTunnels = java.util.Arrays.copyOf(pathTunnelChances, pathTunnelChances.length);
