...
import com.sacredpixel.sacredpixeldungeon.levels.rooms.quest.VaultGateRoom;
import com.sacredpixel.sacredpixeldungeon.levels.rooms.standard.ImpShopRoom;
...
public class CityBossLevel extends Level {
	...
	private static final Rect entry = new Rect(1, 47, 14, 58);
	private static final Rect arena = new Rect(1, 35, 14, 48);
	private static final Rect end = new Rect(0, 0, 15, 32);
	...
	public static int summoningAltar;
	private boolean altarUsed = false;
	...
	private static final String SUMMONING_ALTAR = "summoning_altar";
	private static final String ALTAR_USED = "altar_used";

	@Override
	public void storeInBundle( Bundle bundle ) {
		super.storeInBundle( bundle );
		bundle.put( IMP_SHOP, impShop );
		bundle.put( SUMMONING_ALTAR, summoningAltar );
		bundle.put( ALTAR_USED, altarUsed );
	}
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		summoningAltar = bundle.getInt( SUMMONING_ALTAR );
		altarUsed = bundle.getBoolean( ALTAR_USED );
	...
	protected boolean build() {
		...
		// If boss already defeated, unlock doors
		if (Statistics.bossScores[3] > 0) {
			Painter.set(this, c.x, arena.top, Terrain.DOOR);
			Painter.set(this, bottomDoor, Terrain.DOOR);
		} else {
		...
		Painter.fill(this, end.left+4, end.top+5, 7, 28, Terrain.EMPTY);
		...
		// EMPTY tile at center for interaction (PEDESTAL has special rendering in CustomGroundVisuals)
		...
		Painter.fill(this, end.left+1, end.top+22, 2, 2, Terrain.WALL);
		Painter.fill(this, end.left+1, end.top+27, 2, 2, Terrain.WALL);
		...
		Painter.fill(this, end.right-3, end.top+22, 2, 2, Terrain.WALL);
		Painter.fill(this, end.right-3, end.top+27, 2, 2, Terrain.WALL);
		...
		// Add quest entrance visuals AFTER CustomGroundVisuals so they render on top
		Point altarCenterForVisuals = new Point(end.left+7, end.top+26);
		VaultGateRoom.QuestEntrance vis = new VaultGateRoom.QuestEntrance();
		vis.pos(altarCenterForVisuals.x - 2, altarCenterForVisuals.y - 2);
		customTiles.add(vis);

		VaultGateRoom.EntranceBarrier vis2 = new VaultGateRoom.EntranceBarrier();
		vis2.pos(altarCenterForVisuals.x - 1, altarCenterForVisuals.y - 1);
		customTiles.add(vis2);

		// If boss already defeated and Imp quest completed, spawn shop immediately
		if (Statistics.bossScores[3] > 0 && Imp.Quest.isCompleted()) {
			spawnShop();
		}
			...
			public void call() {
				GameScene.show(new WndOptions(
					...
					protected void onSelect(int index) {
						...
						// index == 1: Cancel, just close the window
					}
				});
			}
		});
	...
	private void summonRatKing() {
		// Find a valid spawn position near the altar
	...
	public static class CustomGroundVisuals extends CustomTilemap {
		...
		public Tilemap create() {
			...
			//upper part of the level, demon halls tiles (including altar room)
			for (int i = tileW; i < tileW*32; i++){
			...
			//lower part: statues, pedestals, and carpets (DK arena and entry)
			for (int i = tileW*32; i < tileW * tileH; i++){
		...
		public String name(int tileX, int tileY) {
			...
			//demon halls tiles (including altar room)
			if (cell < Dungeon.level.width*32){
				if (Dungeon.level.map[cell] == Terrain.STATUE){
					return Messages.get(HallsLevel.class, "statue_name");
				} else if (cell == summoningAltar){
					return Messages.get(CityBossLevel.class, "altar_title");
				}
		...
		public String desc(int tileX, int tileY) {
			...
			//demon halls tiles (including altar room)
			if (cell < Dungeon.level.width*32){
				if (cell == summoningAltar){
					return Messages.get(CityBossLevel.class, "altar_examine");
				} else if (Dungeon.level.map[cell] == Terrain.EXIT){
	...
	public static class CustomWallVisuals extends CustomTilemap {
		...
		public Tilemap create() {
			...
			//lower part. Statues and DK's throne (DK arena and entry)
			for (int i = tileW*31; i < tileW * tileH; i++){
