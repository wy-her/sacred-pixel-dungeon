...
public class Flare extends Visual {
	...
	// Shared indices cache: index patterns depend only on nRays, not radius
	private static final java.util.HashMap<Integer, ShortBuffer> sharedIndices = new java.util.HashMap<>();

	private static ShortBuffer getSharedIndices( int nRays ) {
		ShortBuffer cached = sharedIndices.get( nRays );
		if (cached != null) {
			((Buffer)cached).position( 0 );
			return cached;
		}
	...
	public Flare( int nRays, float radius ) {
		...
		indices = getSharedIndices( nRays );
