...
import com.sacredpixel.sacredpixeldungeon.effects.particles.BloodParticle;
import com.sacredpixel.sacredpixeldungeon.effects.particles.CorrosionParticle;
...
import com.sacredpixel.sacredpixeldungeon.effects.particles.PurpleParticle;
import com.sacredpixel.sacredpixeldungeon.effects.particles.RainbowParticle;
...
public class MagicMissile extends Emitter {
	...
	public void reset( int type, PointF from, PointF to, Callback callback ) {
		...
		//Note: Emitter already applies 2x interval for HTML5 continuous emissions,
		//so we do NOT add an additional multiplier here to avoid 6x compounding.
		float intervalMul = 1f;

		//for now all specks share the same size and volume, this can easily be customized later if needed
		if (type >= SPECK){
			size( 10 );
			pour( Speck.factory(type-SPECK), 0.02f * intervalMul);
			revive();
			return;
		}
		...
		switch(type){
			case MAGIC_MISSILE: default:
				size( 4 );
				pour( WhiteParticle.FACTORY, 0.01f * intervalMul );
				break;
			case FROST:
				pour( MagicParticle.FACTORY, 0.01f * intervalMul );
				break;
			case FIRE:
				size( 4 );
				pour( FlameParticle.FACTORY, 0.01f * intervalMul );
				break;
			case CORROSION:
				size( 3 );
				pour( CorrosionParticle.MISSILE, 0.01f * intervalMul );
				break;
			case FOLIAGE:
				size( 4 );
				pour( LeafParticle.GENERAL, 0.01f * intervalMul );
				break;
			case FORCE:
				pour( SlowParticle.FACTORY, 0.01f * intervalMul );
				break;
			case BEACON:
				pour( ForceParticle.FACTORY, 0.01f * intervalMul );
				break;
			case SHADOW:
				size( 4 );
				pour( ShadowParticle.MISSILE, 0.01f * intervalMul );
				break;
			case RAINBOW:
				size( 4 );
				pour( RainbowParticle.BURST, 0.01f * intervalMul );
				break;
			case EARTH:
				size( 4 );
				pour( EarthParticle.FACTORY, 0.01f * intervalMul );
				break;
			case WARD:
				size( 4 );
				pour( WardParticle.FACTORY, 0.01f * intervalMul );
				break;
			case SHAMAN_RED:
				size( 2 );
				pour( ShamanParticle.RED, 0.01f * intervalMul );
				break;
			case SHAMAN_BLUE:
				size( 2 );
				pour( ShamanParticle.BLUE, 0.01f * intervalMul );
				break;
			case SHAMAN_PURPLE:
				size( 2 );
				pour( ShamanParticle.PURPLE, 0.01f * intervalMul );
				break;
			case ELMO:
				size( 5 );
				pour( ElmoParticle.FACTORY, 0.01f * intervalMul );
				break;
			case POISON:
				size( 3 );
				pour( PoisonParticle.MISSILE, 0.01f * intervalMul );
				break;
			case LIGHT_MISSILE:
				size( 4 );
				pour( WhiteParticle.YELLOW, 0.01f * intervalMul );
				break;

			case MAGIC_MISS_CONE:
				size( 10 );
				pour( WhiteParticle.FACTORY, 0.03f * intervalMul );
				break;
			case FROST_CONE:
				size( 10 );
				pour( MagicParticle.FACTORY, 0.03f * intervalMul );
				break;
			case FIRE_CONE:
				size( 10 );
				pour( FlameParticle.FACTORY, 0.03f * intervalMul );
				break;
			case CORROSION_CONE:
				size( 10 );
				pour( CorrosionParticle.MISSILE, 0.03f * intervalMul );
				break;
			case FOLIAGE_CONE:
				size( 10 );
				pour( LeafParticle.GENERAL, 0.03f * intervalMul );
				break;
			case FORCE_CONE:
				size( 10 );
				pour( SlowParticle.FACTORY, 0.02f * intervalMul );
				break;
			case SHADOW_CONE:
				size( 10 );
				pour( ShadowParticle.MISSILE, 0.03f * intervalMul );
				break;
			case RAINBOW_CONE:
				size( 10 );
				pour( RainbowParticle.BURST, 0.03f * intervalMul );
				break;
			case EARTH_CONE:
				size( 10 );
				pour( EarthParticle.FACTORY, 0.03f * intervalMul );
				break;
			case WARD_CONE:
				size( 10 );
				pour( WardParticle.FACTORY, 0.03f * intervalMul );
				break;
			case PURPLE_CONE:
				size( 10 );
				pour( PurpleParticle.MISSILE, 0.03f * intervalMul );
				break;
			case SPARK_CONE:
				size( 10 );
				pour( SparkParticle.FACTORY, 0.03f * intervalMul );
				break;
			case BLOOD_CONE:
				size( 10 );
				pour( BloodParticle.FACTORY, 0.03f * intervalMul );
				break;
		}
	...
	@Override
	public void kill() {
		//safety: if missile is destroyed mid-flight, release the VFX blocker
		if (blockingVfx) {
			blockingVfx = false;
