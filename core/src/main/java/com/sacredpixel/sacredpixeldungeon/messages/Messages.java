...
package com.sacredpixel.sacredpixeldungeon.messages;
...
import com.watabou.utils.StringCompat;
...
public class Messages {
	...
	public static String get(Class c, String k, Object...args){
		String key;
		if (c != null){
			key = c.getName().replace("com.sacredpixel.sacredpixeldungeon.", "");
			key += "." + k;
		} else
			key = k;
		...
		if (value != null){
			...
			// Unescape %% to % even when no format args are provided
			// Properties files use %% to escape % for String.format(), but
			// when format() isn't called, we need to unescape manually
	...
	public static String format( String format, Object...args ) {
		try {
			return com.watabou.utils.StringCompat.format(locale(), format, args);
		} catch (Exception e) {
			SacredPixelDungeon.reportException( new Exception("formatting error for the string: " + format, e) );
			return format;
		}
	}

	public static String decimalFormat( String format, double number ){
		return StringCompat.decimalFormat(format, number, locale());
	}
