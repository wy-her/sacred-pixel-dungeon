...
import com.sacredpixel.sacredpixeldungeon.actors.buffs.MagicalSight;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.MindVision;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.RevealedArea;
...
import com.sacredpixel.sacredpixeldungeon.actors.hero.spells.DivineSense;
...
import com.sacredpixel.sacredpixeldungeon.levels.CavesBossLevel;
import com.sacredpixel.sacredpixeldungeon.levels.CavesLevel;
import com.sacredpixel.sacredpixeldungeon.levels.CityBossLevel;
import com.sacredpixel.sacredpixeldungeon.levels.CityLevel;
import com.sacredpixel.sacredpixeldungeon.levels.DeadEndLevel;
import com.sacredpixel.sacredpixeldungeon.levels.HallsBossLevel;
import com.sacredpixel.sacredpixeldungeon.levels.HallsLevel;
import com.sacredpixel.sacredpixeldungeon.levels.LastLevel;
import com.sacredpixel.sacredpixeldungeon.levels.Level;
import com.sacredpixel.sacredpixeldungeon.levels.MiningLevel;
import com.sacredpixel.sacredpixeldungeon.levels.PrisonBossLevel;
import com.sacredpixel.sacredpixeldungeon.levels.PrisonLevel;
...
public class Dungeon {
	...
	public static void loadGame( int save, boolean fullLoad ) throws IOException {
		...
		if (fullLoad) {
			...
			generatedLevels.clear();
			int[] genLevels = bundle.getIntArray(GENERATED_LEVELS);
			if (genLevels != null) {
				for (int i : genLevels) {
			...
			droppedItems = new SparseArray<>();
			//check main floors (1-27), and branch floors (1001-1026)
			for (int i=1; i <= 27; i++) {
				for (int b = 0; b <= 1; b++) {
					int key = i + 1000*b;
					//dropped items
					ArrayList<Item> items = new ArrayList<>();
					if (bundle.contains(Messages.format( DROPPED, key )))
						for (Bundlable bun : bundle.getCollection( Messages.format( DROPPED, key ) ) ) {
							items.add( (Item)bun );
						}
					if (!items.isEmpty()) {
						droppedItems.put( key, items );
					}
				}
			}
		}
	...
	public static void fail( Object cause ) {
		if (WndResurrect.instance == null) {
			...
			// Don't record tutorial deaths in rankings
			if (!(level instanceof TutorialLevel)) {
	...
	// Dedup: skip redundant observe() calls within the same frame.
	// Uses a simple counter that increments on each observe() call.
	// GameScene resets this at the start of each frame.
	private static int observeCountThisFrame = 0;
	private static final int MAX_OBSERVES_PER_FRAME = 2;

	public static void resetObserveDedup(){
		observeCountThisFrame = 0;
	}
	...
	public static void observe( int dist ) {
		...
		// On HTML5, limit FOV recalculations per frame to avoid blocking the render thread.
		// The first 2 observe() calls per frame always execute (covers movement + ability use).
		// Additional calls are skipped -- the next frame will pick up any remaining changes.
		if (com.watabou.utils.DeviceCompat.isHTML5()) {
			observeCountThisFrame++;
			if (observeCountThisFrame > MAX_OBSERVES_PER_FRAME) {
				return;
			}
		}
