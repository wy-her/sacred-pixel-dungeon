...
import com.sacredpixel.sacredpixeldungeon.actors.blobs.Foliage;
import com.sacredpixel.sacredpixeldungeon.actors.blobs.SacrificialFire;
import com.sacredpixel.sacredpixeldungeon.actors.blobs.WaterOfAwareness;
import com.sacredpixel.sacredpixeldungeon.actors.blobs.WaterOfHealth;
...
import com.sacredpixel.sacredpixeldungeon.actors.mobs.npcs.ImpShopkeeper;
...
import com.sacredpixel.sacredpixeldungeon.sprites.GhostSprite;
...
import com.sacredpixel.sacredpixeldungeon.sprites.RatKingSprite;
import com.sacredpixel.sacredpixeldungeon.sprites.ShopkeeperSprite;
...
import com.sacredpixel.sacredpixeldungeon.sprites.WandmakerSprite;
...
public class Notes {
		...
		public void restoreFromBundle(Bundle bundle) {
			...
			if (bundle.contains(ITEM_CLASS)) {
				...
				if (type == CustomType.ITEM){
					...
					if (Compat.isAssignableFrom(EquipableItem.class, itemClass)){
						type = CustomType.SPECIFIC_ITEM;
					} else {
						type = CustomType.ITEM_TYPE;
					}
	...
	public static <T extends Record> ArrayList<T> getRecords( Class<T> recordType ){
		...
		for (Record rec : records){
			if (Compat.isInstance(recordType, rec)){
				filtered.add((T)rec);
			}
		}
		return filtered;
