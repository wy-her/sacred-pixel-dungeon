...
public class WndHardNotification extends WndTitledMessage{
	...
	public WndHardNotification(Component titlebar, String message, String btnMessage, int time) {
		...
		btnOkay.setRect(0, height + GAP, width, BTN_HEIGHT);
		btnOkay.enable(false);
		add(btnOkay);
		addFocusableButton(btnOkay);
	...
	public void onConfirm() {
		if (timeLeft <= 0) {
			// Button is enabled, activate it (which calls hide())
			if (focusIndex >= 0 && focusIndex < focusableButtons.size()) {
				activateFocused();
			} else {
				// No button focused, just hide
				hide();
			}
		}
		// If timer not done, ignore Enter key
	}
}
