...
public class WndKeyBindings extends Window {
	...
	//Action column: 46% of width. Key columns: remaining 54% split into 3 (18% each).
	private static final float ACTION_FRAC = 0.46f;
	private static final float KEY_FRAC    = (1f - ACTION_FRAC) / 3f; // 0.18

	private static final int COL1_CENTER = (int)(WIDTH * ACTION_FRAC / 2);
	private static final int COL2_CENTER = (int)(WIDTH * (ACTION_FRAC + KEY_FRAC * 0.5f));
	private static final int COL3_CENTER = (int)(WIDTH * (ACTION_FRAC + KEY_FRAC * 1.5f));
	private static final int COL4_CENTER = (int)(WIDTH * (ACTION_FRAC + KEY_FRAC * 2.5f));

	//Returns the x position of the nth column separator (0=after action, 1=after key1, 2=after key2)
	//Uses integer math to ensure exact pixel alignment between header and rows.
	static int colSepX(int n) {
		int actionW = (int)(WIDTH * ACTION_FRAC);
		int keyW    = (int)(WIDTH * KEY_FRAC);
		return actionW + keyW * n;
	}
	...
	public WndKeyBindings(Boolean controller) {
		...
		//Column pixel positions (shared between header and BindingItems)
		int sepX1 = colSepX(0);
		int sepX2 = colSepX(1);
		int sepX3 = colSepX(2);
		int keyColW = sepX2 - sepX1 - 2;

		RenderedTextBlock ttlAction = PixelScene.renderTextBlock(Messages.get(this, "ttl_action"), 5);
		ttlAction.text(ttlAction.text(), sepX1);
		...
		RenderedTextBlock ttlKey1 = PixelScene.renderTextBlock(Messages.get(this, "ttl_key1"), 5);
		ttlKey1.maxWidth(keyColW);
		ttlKey1.align(RenderedTextBlock.CENTER_ALIGN);
		float ttlKey1Pad = ttlKey1.nLines > 1 ? Math.round(5 * 0.5f) / 2f : 0;
		ttlKey1.setPos(COL2_CENTER - ttlKey1.width()/2, (BTN_HEIGHT - ttlKey1.height())/2 + ttlKey1Pad);
		add(ttlKey1);
		...
		RenderedTextBlock ttlKey2 = PixelScene.renderTextBlock(Messages.get(this, "ttl_key2"), 5);
		ttlKey2.maxWidth(keyColW);
		ttlKey2.align(RenderedTextBlock.CENTER_ALIGN);
		float ttlKey2Pad = ttlKey2.nLines > 1 ? Math.round(5 * 0.5f) / 2f : 0;
		ttlKey2.setPos(COL3_CENTER - ttlKey2.width()/2, (BTN_HEIGHT - ttlKey2.height())/2 + ttlKey2Pad);
		add(ttlKey2);
		...
		RenderedTextBlock ttlKey3 = PixelScene.renderTextBlock(Messages.get(this, "ttl_key3"), 5);
		ttlKey3.maxWidth(keyColW);
		ttlKey3.align(RenderedTextBlock.CENTER_ALIGN);
		float ttlKey3Pad = ttlKey3.nLines > 1 ? Math.round(5 * 0.5f) / 2f : 0;
		ttlKey3.setPos(COL4_CENTER - ttlKey3.width()/2, (BTN_HEIGHT - ttlKey3.height())/2 + ttlKey3Pad);
		add(ttlKey3);
		...
		for (GameAction action : actionList.toArray(new GameAction[0])) {
			...
			//hide FOCUS actions (movement keys handle all focus)
			} else if (action == SPDAction.FOCUS_UP
					|| action == SPDAction.FOCUS_DOWN
					|| action == SPDAction.FOCUS_LEFT
					|| action == SPDAction.FOCUS_RIGHT) {
				actionList.remove(action);

			//hide selector actions (not used in this build)
			} else if (action == SPDAction.INVENTORY_SELECTOR
					|| action == SPDAction.QUICKSLOT_SELECTOR) {
				actionList.remove(action);
		...
		RedButton btnDefaults = new RedButton(Messages.get(this, "default"), 7){
			@Override
		...
		btnConfirm = new RedButton(Messages.get(this, "confirm"), 7){
			@Override
			protected void onClick() {
		...
		int halfL = WIDTH/2;
		int halfR = WIDTH - halfL;
		btnConfirm.setRect(0, height - BTN_HEIGHT, halfL, BTN_HEIGHT);
		add(btnConfirm);

		btnCancel = new RedButton(Messages.get(this, "cancel"), 7){
			@Override
			protected void onClick() {
				hide(); //close and don't save
			}
		};
		btnCancel.setRect(halfL + 1, height - BTN_HEIGHT, halfR - 1, BTN_HEIGHT);
		add(btnCancel);
		...
		addFocusableButton(btnDefaults);
	...
	public boolean onSignal(KeyEvent event) {
		...
		if (event.pressed) {
			...
			if (keyCode == com.badlogic.gdx.Input.Keys.ENTER || keyCode == com.badlogic.gdx.Input.Keys.NUMPAD_ENTER) {
				...
				hide();
				return true;
			} else if (keyCode == com.badlogic.gdx.Input.Keys.ESCAPE || keyCode == com.badlogic.gdx.Input.Keys.BACK) {
				hide();
				return true;
			}
		}
	...
	private class BindingItem extends Component {
		...
		public BindingItem( GameAction action ){
			...
			actionName = PixelScene.renderTextBlock(Messages.get(WndKeyBindings.class, action.name()), 5 );
			...
			key1Name = PixelScene.renderTextBlock( KeyBindings.getKeyName(key1), 5 );
			if (key1 == 0) key1Name.hardlight(UNBOUND);
			add(key1Name);

			key2Name = PixelScene.renderTextBlock( KeyBindings.getKeyName(key2), 5 );
			if (key2 == 0) key2Name.hardlight(UNBOUND);
			add(key2Name);

			key3Name = PixelScene.renderTextBlock( KeyBindings.getKeyName(key3), 5 );
		...
		protected void layout() {
			...
			//Use colSepX() for exact pixel alignment with header separators
			int s1 = colSepX(0);
			int s2 = colSepX(1);
			int s3 = colSepX(2);
			int actionW = s1;
			...
			actionName.maxWidth(actionW);
			key1Name.maxWidth(keyColW);
			key2Name.maxWidth(keyColW);
			key3Name.maxWidth(keyColW);

			//Center text vertically within the row.
			//RenderedTextBlock adds bottom padding (3px for 2-line, 1px for 1-line at size 5).
			//Compensate by shifting multi-line text down by half the extra padding
			//so the visual text (excluding padding) is truly centered.
			float actionPadCompensation = actionName.nLines > 1 ? Math.round(5 * 0.5f) / 2f : 0;
			float key1PadCompensation = key1Name.nLines > 1 ? Math.round(5 * 0.5f) / 2f : 0;
			float key2PadCompensation = key2Name.nLines > 1 ? Math.round(5 * 0.5f) / 2f : 0;
			float key3PadCompensation = key3Name.nLines > 1 ? Math.round(5 * 0.5f) / 2f : 0;
			actionName.setPos( x, y + (height() - actionName.height()) / 2f + actionPadCompensation);
			key1Name.setPos(x + s1 + 1, y + (height() - key1Name.height()) / 2f + key1PadCompensation);
			key2Name.setPos(x + s2 + 1, y + (height() - key2Name.height()) / 2f + key2PadCompensation);
			key3Name.setPos(x + s3 + 1, y + (height() - key3Name.height()) / 2f + key3PadCompensation);

			//Separators: use same colSepX() as header for exact alignment
			sep1.size(width, 1);
			sep1.x = x;
			sep1.y = bottom();
		...
		private boolean onClick( float x, float y ){
			if (inside(x, y)){
				...
				int s3 = colSepX(2);
				//assigning third key
				if (x >= this.x + s3 && key2 != 0) {
					SacredPixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, 3, key3, key1, key2));

				//assigning second key
				} else if (x >= this.x + s2 && key1 != 0) {
					SacredPixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, 2, key2, key1, key3));

				//assigning first key
				} else if (x >= this.x + s1){
					SacredPixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, 1, key1, key2, key3));
	...
	private class WndChangeBinding extends Window {
		...
		public WndChangeBinding(GameAction action, BindingItem listItem, int keyAssigning, int curKeyCode, int otherBoundKey1, int otherBoundKey2 ){
			...
			btnUnbind = new RedButton(Messages.get(this, "unbind"), 7){
				@Override
				protected void onClick() {
					if (action == GameAction.LEFT_CLICK && listItem.key2 == 0 && listItem.key3 == 0){
						SacredPixelDungeon.scene().addToFront(new WndMessage(Messages.get(WndChangeBinding.class, "cant_unbind")));
					} else {
