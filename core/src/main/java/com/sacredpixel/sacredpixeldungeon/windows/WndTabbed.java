...
public class WndTabbed extends Window {
	...
	public WndTabbed() {
		...
		KeyEvent.addKeyListener(tabListener = new Signal.Listener<KeyEvent>() {
			...
			public boolean onSignal(KeyEvent keyEvent) {
				...
				// Don't handle tab cycling if this window is inactive or not topmost
	...
	protected boolean isInsideWindowArea(int screenX, int screenY) {
		// First check chrome area (main window content)
		if (chrome.overlapsScreenPoint(screenX, screenY)) {
			return true;
		}

		// Check tab area - tabs are below chrome, from y=height to y=height+tabHeight()
		// Convert screen coordinates to local camera coordinates
		float localX = (screenX - camera.x) / camera.zoom + camera.scroll.x;
		float localY = (screenY - camera.y) / camera.zoom + camera.scroll.y;

		// Tab area bounds (tabs start from left margin, end at right margin)
	...
	protected class LabeledTab extends Tab {
		...
		protected void createChildren() {
			...
			btLabel = PixelScene.renderTextBlock( 8 );
			add( btLabel );
		}
