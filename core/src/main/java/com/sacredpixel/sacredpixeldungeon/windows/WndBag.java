...
public class WndBag extends WndTabbed {
	...
	protected static int SLOT_WIDTH_P   = 29;
	protected static int SLOT_WIDTH_L   = 29;

	protected static int SLOT_HEIGHT_P	= 29;
	protected static int SLOT_HEIGHT_L	= 29;
	...
	private java.util.ArrayList<InventorySlot> inventorySlots = new java.util.ArrayList<>();
	private java.util.ArrayList<Item> inventoryItems = new java.util.ArrayList<>();
	private int focusedSlotIndex = -1;
	private com.watabou.noosa.ColorBlock slotHighlight;
	...
	private void moveSlotFocus(int dx, int dy) {
		if (inventorySlots.isEmpty()) return;

		int oldIndex = focusedSlotIndex;

		if (focusedSlotIndex == -1) {
			focusedSlotIndex = 0;
		} else {
			int curCol = focusedSlotIndex % nCols;
			int curRow = focusedSlotIndex / nCols;
			int newCol = curCol + dx;
			int newRow = curRow + dy;
			if (newCol < 0) newCol = nCols - 1;
			if (newCol >= nCols) newCol = 0;
			if (newRow < 0) newRow = (inventorySlots.size() - 1) / nCols;
			if (newRow > (inventorySlots.size() - 1) / nCols) newRow = 0;
			int newIndex = newRow * nCols + newCol;
			if (newIndex >= inventorySlots.size()) newIndex = inventorySlots.size() - 1;
	...
	private void updateSlotHighlight() {
		...
		if (focusedSlotIndex >= 0 && focusedSlotIndex < inventorySlots.size()) {
			InventorySlot s = inventorySlots.get(focusedSlotIndex);
			if (slotHighlight == null) {
				slotHighlight = new com.watabou.noosa.ColorBlock(1, 1, 0x44FFFFFF);
				add(slotHighlight);
			}
			slotHighlight.size(s.width(), s.height());
			slotHighlight.x = s.left();
			slotHighlight.y = s.top();
			slotHighlight.visible = true;
		}
	}

	private void activateFocusedSlot() {
		if (focusedSlotIndex >= 0 && focusedSlotIndex < inventorySlots.size()) {
			InventorySlot s = inventorySlots.get(focusedSlotIndex);
			if (!s.active) return;
			Item item = inventoryItems.get(focusedSlotIndex);
