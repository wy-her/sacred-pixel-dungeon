...
import com.sacredpixel.sacredpixeldungeon.effects.DarkBlock;
...
import com.sacredpixel.sacredpixeldungeon.effects.IceBlock;
import com.sacredpixel.sacredpixeldungeon.effects.GlowBlock;
import com.sacredpixel.sacredpixeldungeon.effects.ShieldHalo;
...
import com.sacredpixel.sacredpixeldungeon.effects.particles.SnowParticle;
...
public class CharSprite extends MovieClip implements Tweener.Listener, MovieClip.Listener {
	...
	//Force-completes any in-progress movement animation immediately
	...
	public void onComplete( Tweener tweener ) {
		if (tweener == jumpTweener) {
			...
			synchronized (this) {
				...
				GameScene.sortMobSprites();
				ThreadCompat.notifyAllOnObject(this);
			}

		}
	}

	//Duration of visual pause after each attack animation, so consecutive
	//attacks (e.g. hero hit then mob retaliation) appear sequential rather
	//than simultaneous.  Only affects visual pacing, not game-time mechanics.
	private static final float ATTACK_VISUAL_DELAY = 0.1f;
	...
	public synchronized void onComplete( Animation anim ) {
		...
		if (animCallback != null) {
			...
			if (anim == attack) {
				...
				//Add a brief visual delay after hero attack animations so that
				//the hero's attack and the mob's retaliation look distinct.
				//Only applies to hero attacks to avoid slowing down mob sequences.
				if (parent != null && visible && ch == Dungeon.hero) {
					Actor.addVfxBlocker();
					parent.add(new Delayer(ATTACK_VISUAL_DELAY) {
						@Override
