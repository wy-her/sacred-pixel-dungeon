...
import com.sacredpixel.sacredpixeldungeon.windows.WndHardNotification;
...
public class WelcomeScene extends PixelScene {
	...
	private static final int LATEST_UPDATE = SacredPixelDungeon.v3_3_0;
	...
	public void create() {
		...
		if (!triedCleaningTemp && FileUtils.cleanTempFiles()){
			add(new WndHardNotification(Icons.get(Icons.WARNING),
				...
				public void hide() {
					...
					SacredPixelDungeon.resetScene();
				}
			});
			return;
		}

		if (SacredPixelDungeon.versionCode == previousVersion && !SPDSettings.intro()) {
		...
		StyledButton okay = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				if (previousVersion == 0 || SPDSettings.intro()){
					...
					SPDSettings.version(SacredPixelDungeon.versionCode);
		...
		float buttonY = insets.top + Math.min(topRegion + (PixelScene.landscape() ? 60 : 120), h - BTN_HEIGHT - 4);
		...
		if (previousVersion != 0 && !SPDSettings.intro()){
			StyledButton changes = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(TitleScene.class, "changes")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					SacredPixelDungeon.switchScene(ChangesScene.class);
				}
			};
			okay.setRect(btnAreaLeft, buttonY, (buttonAreaWidth/2)-1, BTN_HEIGHT);
			add(okay);

			changes.setRect(okay.right()+1, buttonY, okay.width(), BTN_HEIGHT);
			changes.icon(Icons.get(Icons.CHANGES));
			add(changes);
		} else {
			okay.text(Messages.get(TitleScene.class, "enter"));
			okay.setRect(btnAreaLeft, buttonY, buttonAreaWidth, BTN_HEIGHT);
		...
		} else if (previousVersion <= SacredPixelDungeon.versionCode) {
	...
	private void updateVersion(int previousVersion){
		...
		if (previousVersion < LATEST_UPDATE){
			...
			if (previousVersion <= SacredPixelDungeon.v2_5_4){
