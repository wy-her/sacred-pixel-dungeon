...
import com.sacredpixel.sacredpixeldungeon.windows.WndGameInProgress;
...
public class StartScene extends PixelScene {
	...
	private static final int SLOT_WIDTH = 135;
	...
	public void create() {
		...
		for (GamesInProgress.Info game : games) {
			...
			align(existingGame);
			add(existingGame);
			focusableButtons.add(existingGame);
		}
		...
		if (games.size() >= 2) {
			add(btnSort);
			focusableButtons.add(btnSort);
		}

		// Add exit button to focusable list
		...
		keyListener = new Signal.Listener<KeyEvent>() {
			...
			public boolean onSignal(KeyEvent event) {
				...
				for (Object v : members) {
					if (v instanceof com.sacredpixel.sacredpixeldungeon.ui.Window) return false;
				}
	...
	private static class SaveSlotButton extends Button {
		...
		protected void layout() {
			...
			if (hero != null){
				...
				name.setPos(
						hero.x + hero.width() + 6,
						y + (height - name.height() - lastPlayed.height())/2f
				);
				align(name);
