...
import com.sacredpixel.sacredpixeldungeon.ui.Tooltip;
import com.sacredpixel.sacredpixeldungeon.ui.Window;
import com.sacredpixel.sacredpixeldungeon.utils.Holiday;
...
public class PixelScene extends Scene {
		...
		if (DeviceCompat.isDesktop() && fullscreenListener == null){
			KeyEvent.addKeyListener(fullscreenListener = new Signal.Listener<KeyEvent>() {
				...
				public boolean onSignal(KeyEvent keyEvent) {
					...
					} else if (keyEvent.code == Input.Keys.ENTER || keyEvent.code == Input.Keys.NUMPAD_ENTER){
	...
	// Check if any window is currently open on this scene
	...
	// Override addToFront to immediately deactivate other windows when a new Window is added.
	// This prevents input from being processed by background windows in the same frame,
	// before the next update() cycle calls Window.onAddedToScene().
	// DO NOT REMOVE - this is essential for proper window activation stack timing.
	@Override
	public synchronized Gizmo addToFront(Gizmo g) {
		Gizmo result = super.addToFront(g);

		// When a Window is added, immediately deactivate all other Windows
		if (g instanceof Window) {
			for (Gizmo member : members) {
				if (member instanceof Window && member != g && member.exists) {
					member.active = false;
				}
			}
		}
