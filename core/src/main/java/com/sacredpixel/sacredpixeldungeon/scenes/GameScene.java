...
import com.sacredpixel.sacredpixeldungeon.items.Honeypot;
...
import com.sacredpixel.sacredpixeldungeon.sprites.DiscardedItemSprite;
...
import com.sacredpixel.sacredpixeldungeon.tiles.DungeonTileSheet;
import com.sacredpixel.sacredpixeldungeon.tiles.DungeonTilemap;
import com.sacredpixel.sacredpixeldungeon.tiles.DungeonWallsTilemap;
import com.sacredpixel.sacredpixeldungeon.tiles.FogOfWar;
import com.sacredpixel.sacredpixeldungeon.tiles.GridTileMap;
...
import com.sacredpixel.sacredpixeldungeon.ui.LootIndicator;
import com.sacredpixel.sacredpixeldungeon.ui.MenuPane;
import com.sacredpixel.sacredpixeldungeon.ui.QuickSlotButton;
import com.sacredpixel.sacredpixeldungeon.ui.ResumeIndicator;
...
import com.sacredpixel.sacredpixeldungeon.ui.Tag;
import com.sacredpixel.sacredpixeldungeon.ui.TargetHealthIndicator;
import com.sacredpixel.sacredpixeldungeon.ui.Toast;
...
import com.sacredpixel.sacredpixeldungeon.windows.WndInfoCell;
import com.sacredpixel.sacredpixeldungeon.windows.WndInfoItem;
import com.sacredpixel.sacredpixeldungeon.windows.WndInfoMob;
import com.sacredpixel.sacredpixeldungeon.windows.WndInfoPlant;
import com.sacredpixel.sacredpixeldungeon.windows.WndInfoTrap;
...
public class GameScene extends PixelScene {
	...
	public void create() {
		...
		if (InterlevelScene.mode != InterlevelScene.Mode.NONE) {
			...
			} else if (!TutorialManager.isTutorialLevel()) {
		...
		//if the hero is already ready (e.g. returning from AlchemyScene),
		//we need to set the default cell listener on the new CellSelector
	...
	public void destroy() {
		...
		//On HTML5 there is no real actor thread, so skip this entirely.
	...
	public static void endActorThread(){
		if (DeviceCompat.isHTML5()) return;
	...
	public boolean waitForActorThread(int msToWait, boolean interrupt){
		if (DeviceCompat.isHTML5()) return true;
		if (actorThread == null || !actorThread.isAlive()) {
			return true;
		}
		synchronized (actorThread) {
			...
			try {
				ThreadCompat.waitOnObject(actorThread, msToWait);
	...
	public synchronized void update() {
		...
		if (!Actor.processing() && Dungeon.hero.isAlive()) {
			if (DeviceCompat.isHTML5()) {
				//On HTML5, process actors synchronously - no threading
				Actor.process();
				...
				if (com.watabou.utils.ThreadCompat.availableProcessors() == 1) {
				...
				notifyDelay += 1/60f;
				synchronized (actorThread) {
					ThreadCompat.notifyAllOnObject(actorThread);
				}
			}
		}
	...
	public static synchronized SpellSprite spellSprite() {
		if (scene != null) return (SpellSprite)scene.spells.recycle( SpellSprite.class );
		return null;
	}
	...
	public static void flashForDocument( Document doc, String page ){
		if (scene != null) {
			// Show the guide directly instead of flashing journal button
			if (doc.pageNames().contains(page)){
				GameScene.show( new WndStory( doc.pageSprite(page),
						doc.pageTitle(page),
						doc.pageBody(page) ){
					@Override
					public void hide() {
	...
	public static void endIntro(){
		if (scene != null){
			...
			GLog.newLine(); // Add line break before tutorial journal hint
	...
	public static void show( Window wnd ) {
		if (scene != null) {
			// Block other windows while WndStory is open (dungeon guide)
			if (!(wnd instanceof WndStory) && showingWndStory()) {
				return;
			}
	...
	public static boolean showingWndStory(){
		if (scene == null) return false;

		for (Gizmo g : scene.members){
			if (g instanceof WndStory) return true;
		}
	...
	public static void gameOver() {
		...
		StyledButton menu = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(WndKeyBindings.class, "menu"), 8){
			@Override
			protected void onClick() {
		...
		// Add keyboard navigation for death screen buttons
		final StyledButton[] buttons = { restart, menu };
		final int[] focusIndex = { -1 };  // Using array for closure
		final boolean[] buttonsFocusable = { false };  // Track when buttons are visible enough

		Signal.Listener<KeyEvent> deathKeyListener = new Signal.Listener<KeyEvent>() {
			@Override
			public boolean onSignal(KeyEvent event) {
				if (!event.pressed) return false;

				// Only handle input once buttons are visible (alpha > 0.5)
				if (gameOver.am < 0.7f) return false;
				buttonsFocusable[0] = true;

				GameAction action = KeyBindings.getActionForKey(event);

				if (action == SPDAction.N || action == SPDAction.NW || action == SPDAction.NE
						|| action == SPDAction.W) {
					// Move focus up (with wrap-around)
					if (focusIndex[0] >= 0) buttons[focusIndex[0]].setFocused(false);
					focusIndex[0]--;
					if (focusIndex[0] < 0) focusIndex[0] = buttons.length - 1;
					buttons[focusIndex[0]].setFocused(true);
					return true;
				} else if (action == SPDAction.S || action == SPDAction.SW || action == SPDAction.SE
						|| action == SPDAction.E) {
					// Move focus down (with wrap-around)
					if (focusIndex[0] >= 0) buttons[focusIndex[0]].setFocused(false);
					focusIndex[0]++;
					if (focusIndex[0] >= buttons.length) focusIndex[0] = 0;
					buttons[focusIndex[0]].setFocused(true);
					return true;
				} else if (event.code == com.badlogic.gdx.Input.Keys.ENTER
						|| event.code == com.badlogic.gdx.Input.Keys.NUMPAD_ENTER) {
					if (focusIndex[0] >= 0 && focusIndex[0] < buttons.length) {
						Sample.INSTANCE.play(Assets.Sounds.CLICK);
						buttons[focusIndex[0]].click();
					}
					return true;
				}
				return false;
		...
		KeyEvent.addKeyListener(deathKeyListener);
	}
	...
	public static void selectCell( CellSelector.Listener listener ) {
		...
		//auto-init targeting cursor only when actually targeting (not for default listener)
		if (listener != defaultCellListener) {
			cellSelector.initKeyboardCursor();
		}
	}
	
	public static void moveCellSelectorCursorTo(int cell) {
		if (cellSelector != null && cellSelector.isTargeting()) {
			cellSelector.moveCursorTo(cell);
		}
	}

	public static boolean confirmCellSelector() {
		if (cellSelector != null && cellSelector.isTargeting()) {
			cellSelector.confirmKeyboardCursor();
			return true;
		}
		return false;
	}
	...
	public static final CellSelector.Listener defaultCellListener = new CellSelector.Listener() {
		...
		public void onRightClick(Integer cell) {
			...
			if (objects.size() > 1){
				textLines.add(0, textLines.remove(0) + ": " + textLines.get(0));
				for (int i = 1; i < textLines.size(); i++){
					textLines.add(i, Messages.get(GameScene.class, "examine") + ": " + textLines.remove(i));
				}
			} else {
				// Keep text without markdown highlighting (will use button's textColor)
				textLines.add(0, textLines.remove(0));
				textLines.add(1, Messages.get(GameScene.class, "examine"));
			}
