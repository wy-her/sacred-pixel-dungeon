...
import com.sacredpixel.sacredpixeldungeon.actors.hero.HeroAction;
...
public class CellSelector extends ScrollArea {
	...
	//keyboard targeting cursor
	private int keyboardCursorCell = -1;
	private com.watabou.noosa.Visual keyboardCursor;
	...
	protected void onClick( PointerEvent event ) {
		if (Dungeon.hero != null && !Dungeon.hero.ready) {
			...
			// Attack and Interact are excluded as they have different action semantics.
			HeroAction action = Dungeon.hero.curAction;
			if (action != null
					&& !(action instanceof HeroAction.Attack)
					&& !(action instanceof HeroAction.Interact)
					&& action.dst != Dungeon.hero.pos) {
				Dungeon.hero.interrupt();
			}
			return;
		}
	...
	protected void onPointerDown( PointerEvent event ) {
		...
		// Block pointer down when windows are open
		if (GameScene.interfaceBlockingHero()) {
			return;
		}
	...
	protected void onDrag( PointerEvent event ) {
		// Block drag when windows are open
		if (GameScene.interfaceBlockingHero()) {
			return;
		}
	...
	//used for continuous camera panning
	private GameAction heldCameraAction = SPDAction.NONE;
	private float cameraPanDelay = 0f;
	...
	public void initKeyboardCursor(){
		if (keyboardCursorCell == -1 && Dungeon.hero != null) {
			//first check if the listener specifies an initial cell (e.g., beneficial spells)
			...
			//if there's a visible enemy target, start cursor there
				...
				keyboardCursorCell = QuickSlotButton.lastTarget.pos;
			} else {
				keyboardCursorCell = Dungeon.hero.pos;
			}
		}
		updateKeyboardCursor();
	}

	private void moveKeyboardCursor(Point dir){
		if (keyboardCursorCell == -1) {
			initKeyboardCursor();
			return;
		}
		int newCell = keyboardCursorCell + dir.x + dir.y * Dungeon.level.width();
		if (newCell >= 0 && newCell < Dungeon.level.length()) {
			keyboardCursorCell = newCell;
			updateKeyboardCursor();
		}
	}

	private void updateKeyboardCursor(){
		if (keyboardCursor == null) {
			keyboardCursor = Icons.TARGET.get();
		}
		if (keyboardCursor.parent != parent) {
			if (keyboardCursor.parent != null) keyboardCursor.remove();
			parent.addToFront(keyboardCursor);
		}
		if (keyboardCursorCell >= 0) {
			keyboardCursor.visible = true;
			//center on char sprite if one exists at cursor cell
			Char ch = Actor.findChar(keyboardCursorCell);
			if (ch != null && ch.sprite != null) {
				PointF sc = ch.sprite.center();
				keyboardCursor.x = sc.x - keyboardCursor.width / 2f;
				keyboardCursor.y = sc.y - keyboardCursor.height / 2f;
			} else {
				PointF p = DungeonTilemap.tileCenterToWorld(keyboardCursorCell);
				keyboardCursor.x = p.x - keyboardCursor.width / 2f;
				keyboardCursor.y = p.y - keyboardCursor.height / 2f;
			}
			//pan camera to cursor
			Camera.main.panTo(DungeonTilemap.tileCenterToWorld(keyboardCursorCell), 5f);
		}
	}

	public void hideKeyboardCursor(){
		keyboardCursorCell = -1;
		if (keyboardCursor != null) {
			keyboardCursor.visible = false;
		}
	}

	public void moveCursorTo(int cell){
		keyboardCursorCell = cell;
		updateKeyboardCursor();
	}

	public void confirmKeyboardCursor(){
		if (keyboardCursorCell >= 0) {
			int cell = keyboardCursorCell;
			hideKeyboardCursor();
	...
	private Signal.Listener<KeyEvent> keyListener = new Signal.Listener<KeyEvent>() {
		...
		public boolean onSignal(KeyEvent event) {
			...
			//keyboard targeting: movement keys move cursor, CYCLE cycles enemies, enter/quickslot confirms
			if (isTargeting() && event.pressed) {
				Point dir = directionFromAction(action);
				if (!dir.isZero()) {
					moveKeyboardCursor(dir);
					return true;
				}
				...
				if (action == SPDAction.WAIT_OR_PICKUP
					|| event.code == com.badlogic.gdx.Input.Keys.ENTER
					|| event.code == com.badlogic.gdx.Input.Keys.NUMPAD_ENTER
					|| isQuickslotAction(action)) {
					// Don't consume Enter/Numpad Enter if a Window is open - let Window handle it
			...
			if (isCameraPanAction(action)) {
				if (event.pressed) {
					heldCameraAction = action;
					handleCameraPan(action);
					cameraPanDelay = initialDelay();
				} else if (heldCameraAction == action) {
					heldCameraAction = SPDAction.NONE;
				}
				return true;
			}
	...
	public void update() {
		...
		//continuously track char sprite for targeting cursor
		if (keyboardCursorCell >= 0 && keyboardCursor != null && keyboardCursor.visible) {
			Char ch = Actor.findChar(keyboardCursorCell);
			if (ch != null && ch.sprite != null) {
				PointF sc = ch.sprite.center();
				keyboardCursor.x = sc.x - keyboardCursor.width / 2f;
		...
		//continuous camera panning when key is held
		if (heldCameraAction != SPDAction.NONE) {
			cameraPanDelay -= Game.elapsed;
			if (cameraPanDelay <= 0) {
				handleCameraPan(heldCameraAction);
				cameraPanDelay = 0.05f;
			}
		}
	...
	private boolean isQuickslotAction(GameAction action){
		return action == SPDAction.QUICKSLOT_1 || action == SPDAction.QUICKSLOT_2
			|| action == SPDAction.QUICKSLOT_3 || action == SPDAction.QUICKSLOT_4
			|| action == SPDAction.QUICKSLOT_5 || action == SPDAction.QUICKSLOT_6;
	}

	private int enemyCycleIndex = 0;

	private void cycleTargetEnemy(){
		if (Dungeon.hero.visibleEnemies() > 0) {
			Mob target = Dungeon.hero.visibleEnemy(++enemyCycleIndex);
			QuickSlotButton.target(target);
			keyboardCursorCell = target.pos;
			updateKeyboardCursor();
		}
	}

	private boolean isCameraPanAction(GameAction action) {
		return action == SPDAction.CAMERA_UP || action == SPDAction.CAMERA_DOWN
			|| action == SPDAction.CAMERA_LEFT || action == SPDAction.CAMERA_RIGHT
			|| action == SPDAction.CAMERA_UP_LEFT || action == SPDAction.CAMERA_UP_RIGHT
			|| action == SPDAction.CAMERA_DOWN_LEFT || action == SPDAction.CAMERA_DOWN_RIGHT
			|| action == SPDAction.CAMERA_HERO;
	}

	private void handleCameraPan(GameAction action) {
		if (action == SPDAction.CAMERA_HERO) {
			if (Dungeon.hero != null && Dungeon.hero.sprite != null) {
				camera.panTo(Dungeon.hero.sprite.center(), 5f);
			}
			return;
		}

		float panAmount = 32f / camera.zoom;
		float dx = 0, dy = 0;

		if (action == SPDAction.CAMERA_UP)         dy = -panAmount;
		if (action == SPDAction.CAMERA_DOWN)       dy = panAmount;
		if (action == SPDAction.CAMERA_LEFT)       dx = -panAmount;
		if (action == SPDAction.CAMERA_RIGHT)      dx = panAmount;
		if (action == SPDAction.CAMERA_UP_LEFT)    { dx = -panAmount; dy = -panAmount; }
		if (action == SPDAction.CAMERA_UP_RIGHT)   { dx = panAmount;  dy = -panAmount; }
		if (action == SPDAction.CAMERA_DOWN_LEFT)  { dx = -panAmount; dy = panAmount; }
		if (action == SPDAction.CAMERA_DOWN_RIGHT) { dx = panAmount;  dy = panAmount; }

		camera.panFollow(null, 0f);
		camera.shift(new PointF(dx, dy));
	}
	...
	public static abstract class Listener {
		...
		//Beneficial spells can override this to return Dungeon.hero.pos.
