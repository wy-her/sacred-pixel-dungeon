...
import com.sacredpixel.sacredpixeldungeon.services.news.NewsArticle;
...
public class NewsScene extends PixelScene {
	...
	public void create() {
		...
		// "Read more" button removed - no longer linking to external site
	...
	private static class ArticleButton extends StyledButton {
		...
		protected void onClick() {
			...
			SacredPixelDungeon.scene().addToFront(new WndArticle(article));
		}
	}
	...
	private static class WndArticle extends WndTitledMessage {
		...
		public WndArticle(NewsArticle article ) {
			...
			RedButton link = new RedButton(Messages.get(NewsScene.class, "read_more")){
				...
				protected void onClick() {
					super.onClick();
					SacredPixelDungeon.platform.openURI(article.URL);
				}
			};
