...
import com.sacredpixel.sacredpixeldungeon.sprites.GooSprite;
...
public class Goo extends Mob {
	...
	protected boolean doAttack( Char enemy ) {
		if (pumpedUp == 1) {
			...
			if (visible) {
				// Interrupt hero's auto-movement immediately when attack animation starts
	...
	public void die( Object cause ) {
		...
		for (int i = 0; i < blobs; i++){
			...
			} while (--tries > 0 && !Dungeon.level.passable[pos + ofs]);
