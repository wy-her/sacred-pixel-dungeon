...
public class DwarfKing extends Mob {
	...
	private boolean teleportSubject(){
		...
		if (furthest != null){
			...
			int targetCell = trajectory.dist+1 < trajectory.path.size()
					? trajectory.path.get(trajectory.dist+1)
					: trajectory.path.get(trajectory.dist);
		...
		public boolean act() {
			...
			if (delay <= 0){
				...
				if (summon == DKGolem.class){
					if (particles != null) particles.burst(SparkParticle.FACTORY, 10);
					Sample.INSTANCE.play(Assets.Sounds.CHARGEUP);
				} else if (summon == DKWarlock.class){
					if (particles != null) particles.burst(ShadowParticle.CURSE, 10);
					Sample.INSTANCE.play(Assets.Sounds.CURSED);
				} else if (summon == DKMonk.class){
					if (particles != null) particles.burst(ElmoParticle.FACTORY, 10);
					Sample.INSTANCE.play(Assets.Sounds.BURNING);
				} else {
					if (particles != null) particles.burst(Speck.factory(Speck.BONE), 10);
