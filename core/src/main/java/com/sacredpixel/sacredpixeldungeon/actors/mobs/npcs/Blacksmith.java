...
import com.sacredpixel.sacredpixeldungeon.windows.WndBlacksmith;
			...
			if (!node.isNull() && (spawned = node.getBoolean( SPAWNED ))) {
				...
				if (node.contains( SMITH_REWARDS )){
					try {
						...
						Collection<Item> items = (Collection<Item>) (Collection<?>) node.getCollection( SMITH_REWARDS );
						smithRewards = new ArrayList<>(items);
						if (node.contains(ENCHANT)) {
							smithEnchant = (Weapon.Enchantment) node.get(ENCHANT);
							smithGlyph   = (Armor.Glyph) node.get(GLYPH);
						}
					} catch (ClassCastException e) {
						// Corrupted save data - reset smith rewards
						smithRewards = null;
						smithEnchant = null;
						smithGlyph = null;
					}
