...
public abstract class Elemental extends Mob {
	...
	public static class FireElemental extends Elemental {
		
		{
			...
			harmfulBuffs.add( com.sacredpixel.sacredpixeldungeon.actors.buffs.Frost.class );
	...
	public static class NewbornFireElemental extends FireElemental {
		...
		protected boolean act() {
			...
			if (targetingPos != -1 && state == HUNTING){
				...
				// Use heroFOV (game state) instead of sprite.visible (render state)
				// to avoid FOV boundary race conditions on HTML5
				boolean inFOV = Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[targetingPos];
				if (sprite != null && inFOV) {
