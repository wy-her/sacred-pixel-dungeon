...
import com.sacredpixel.sacredpixeldungeon.sprites.RipperSprite;
	...
	public class Hunting extends Mob.Hunting {
		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
			...
			if (leapPos != -1){
				...
				//do leap - spend time before animation so hero cannot act during leap
				spend(attackDelay());
				...
				sprite.jump(pos, leapPos, new Callback() {
					...
					public void call() {
						...
						//re-check if someone moved to our landing spot during the jump
						Char blocker = Actor.findChar(endPos);
						if (blocker != null && blocker != RipperDemon.this) {
							int pushPos = -1;
							for (int i : PathFinder.NEIGHBOURS8) {
								int cell = endPos + i;
