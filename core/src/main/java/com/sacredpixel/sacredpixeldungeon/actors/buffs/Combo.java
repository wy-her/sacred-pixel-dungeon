...
import com.sacredpixel.sacredpixeldungeon.windows.WndCombo;
...
public class Combo extends Buff implements ActionIndicator.Action {
	...
	public boolean act() {
		...
		spend(TICK);
		// Original simple logic - static fields preserve state even if Combo detaches
		if (comboTime <= 0) {
			detach();
		}
		return true;
	...
	private static final String FURY_HITS_LEFT = "fury_hits_left";
	...
	public void storeInBundle(Bundle bundle) {
		...
		// Save FURY state to prevent loss on save/load during multi-hit sequence
		bundle.put(FURY_HITS_LEFT, furyHitsLeft);
		if (moveBeingUsed != null) {
			bundle.put(MOVE_BEING_USED, moveBeingUsed.name());
		}
	}
	...
	public void restoreFromBundle(Bundle bundle) {
		...
		// Restore FURY state to continue multi-hit sequence after load
		furyHitsLeft = bundle.getInt(FURY_HITS_LEFT);
		if (bundle.contains(MOVE_BEING_USED)) {
			moveBeingUsed = ComboMove.valueOf(bundle.getString(MOVE_BEING_USED));
		}
	...
	public static class ParryTracker extends FlavourBuff{
		{ actPriority = MOB_PRIO-1;}  // Must act AFTER mobs attack, not before
	...
	// Original Shattered PD uses static fields - this is correct because
	// there's only ever one Hero with one Combo buff at a time
	private static ComboMove moveBeingUsed;
	private static int furyHitsLeft = 0;

	private void doAttack(final Char enemy) {

		// HTML5 safety: Ensure FURY state consistency across async callbacks
		// furyHitsLeft > 0 indicates we're in the middle of a FURY multi-hit sequence
		// Restore moveBeingUsed in case it was cleared between callback invocations
		if (furyHitsLeft > 0) {
			moveBeingUsed = ComboMove.FURY;
		}
		...
		switch(moveBeingUsed){
			...
			case FURY:
				if (count > 0){
					furyHitsLeft = count;
					count = 0;
					// Don't spend here - wait until chain completes to avoid enemies acting mid-chain
				}
				furyHitsLeft--;
				//fury attacks as many times as you have combo count
				if (furyHitsLeft > 0 && hero.isAlive() && enemy.isAlive() &&
						hero.canAttack(enemy) && (wasAlly || enemy.alignment != target.alignment)){
					target.sprite.attack(enemy.pos, new Callback() {
						@Override
						public void call() {
	...
	private CellSelector.Listener listener = new CellSelector.Listener() {
		...
		public void onSelect(Integer cell) {
			...
			} else if (target.isCharmedBy( enemy )) {
