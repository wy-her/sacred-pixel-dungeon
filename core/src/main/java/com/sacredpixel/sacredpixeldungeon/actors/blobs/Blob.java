...
public class Blob extends Actor {
	...
	protected void evolve() {
		...
		int newTop = area.bottom;
		int newBottom = area.top;
		int newLeft = area.right;
		int newRight = area.left;
			...
			for (int j = area.left-1; j <= area.right; j++) {
				...
				if (Dungeon.level.insideMap(cell)) {
					if (!blocking[cell]) {
						...
						if (value > 0){
							if (i < newTop)    newTop = i;
							if (i >= newBottom) newBottom = i+1;
							if (j < newLeft)   newLeft = j;
							if (j >= newRight)  newRight = j+1;
						}
