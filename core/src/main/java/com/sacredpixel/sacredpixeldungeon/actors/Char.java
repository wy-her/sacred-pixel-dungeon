...
import com.sacredpixel.sacredpixeldungeon.actors.buffs.FrostImbue;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.Fury;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.Haste;
...
import com.sacredpixel.sacredpixeldungeon.actors.buffs.Preparation;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.ShieldBuff;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.Sleep;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.Slow;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.SnipersMark;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.Speed;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.Stamina;
import com.sacredpixel.sacredpixeldungeon.actors.buffs.Terror;
...
import com.sacredpixel.sacredpixeldungeon.actors.hero.abilities.rogue.DeathMark;
import com.sacredpixel.sacredpixeldungeon.actors.hero.abilities.warrior.Endure;
import com.sacredpixel.sacredpixeldungeon.actors.hero.spells.AuraOfProtection;
import com.sacredpixel.sacredpixeldungeon.actors.hero.spells.BeamingRay;
import com.sacredpixel.sacredpixeldungeon.actors.hero.spells.GuidingLight;
import com.sacredpixel.sacredpixeldungeon.actors.hero.spells.LifeLinkSpell;
import com.sacredpixel.sacredpixeldungeon.actors.hero.spells.ShieldOfLight;
...
import com.sacredpixel.sacredpixeldungeon.items.armor.curses.Bulk;
import com.sacredpixel.sacredpixeldungeon.items.armor.glyphs.AntiMagic;
import com.sacredpixel.sacredpixeldungeon.items.armor.glyphs.Brimstone;
import com.sacredpixel.sacredpixeldungeon.items.armor.glyphs.Flow;
import com.sacredpixel.sacredpixeldungeon.items.armor.glyphs.Obfuscation;
import com.sacredpixel.sacredpixeldungeon.items.armor.glyphs.Potential;
import com.sacredpixel.sacredpixeldungeon.items.armor.glyphs.Swiftness;
...
import com.sacredpixel.sacredpixeldungeon.items.weapon.missiles.darts.ShockingDart;
...
import com.sacredpixel.sacredpixeldungeon.sprites.MobSprite;
...
public abstract class Char extends Actor {
	...
	//tracks whether this char was recently visible to the hero (e.g. during an
	//attack animation). Used by moveSprite to ensure movement is animated even
	//if heroFOV has shifted away by the time the char moves next turn.
	...
	protected boolean moveSprite( int from, int to ) {
		...
		boolean inFOV = Dungeon.level.heroFOV[from] || Dungeon.level.heroFOV[to];

		//if the char was recently visible (e.g. just played an attack animation
		//that the player saw), animate the movement even if heroFOV has since
		//shifted away due to the hero auto-moving.
		if (sprite.isVisible() && sprite.parent != null && (inFOV)) {
			sprite.move( from, to );
			return true;
		} else {
	...
	public boolean attack( Char enemy, float dmgMulti, float dmgBonus, float accMulti ) {
		...
		if (enemy.isInvulnerable(getClass())) {
			...
			// Even on a miss/dodge, interrupt hero's continuous movement.
			// Without this, during auto-walk the hero keeps moving while
			// enemies attack, causing enemies to appear to teleport.
	...
	public void damage( int dmg, Object src ) {
		...
		//negative damage of -1 is used as a signal for on-damage effects (e.g. assassination)
		//allow it through but clamp to 0 so no actual HP change occurs
		if (dmg < 0) {
			if (dmg == -1) {
				dmg = 0;
			} else {
