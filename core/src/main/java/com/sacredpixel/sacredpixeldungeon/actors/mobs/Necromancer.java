...
import com.sacredpixel.sacredpixeldungeon.sprites.SkeletonSprite;
...
public class Necromancer extends Mob {
	...
	public void summonMinion(){
		if (Actor.findChar(summoningPos) != null) {
			...
			//first try to push the blocker away from the summoning position
			int pushPos = pos;
			for (int c : PathFinder.NEIGHBOURS8) {
				int candidate = summoningPos + c;
				if (Dungeon.level.insideMap(candidate)
						&& Actor.findChar(candidate) == null
						&& Dungeon.level.passable[candidate]
						&& (Dungeon.level.openSpace[candidate] || !hasProp(Actor.findChar(summoningPos), Property.LARGE))
						&& Dungeon.level.trueDistance(pos, candidate) > Dungeon.level.trueDistance(pos, pushPos)) {
					pushPos = candidate;
				}
			}
			...
			if (pushPos != pos) {
				...
				//if pushing failed, try to find an alternative summon position
				int altPos = -1;
				for (int c : PathFinder.NEIGHBOURS8) {
					int candidate = summoningPos + c;
					if (Dungeon.level.insideMap(candidate)
							&& Actor.findChar(candidate) == null
							&& Dungeon.level.passable[candidate]
							&& (Dungeon.level.openSpace[candidate] || !hasProp(this, Property.LARGE))) {
						if (altPos == -1 || Dungeon.level.trueDistance(pos, candidate) < Dungeon.level.trueDistance(pos, altPos)) {
							altPos = candidate;
						}
					}
				}
				...
				if (!fieldOfView[mySkeleton.pos]){
					...
					if (!Dungeon.level.adjacent(mySkeleton.pos, enemy.pos)){
						...
						if (telePos != -1){
							...
							boolean inFOV = Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[telePos];
					...
					if (mySkeleton.HP < mySkeleton.HT || mySkeleton.buff(Adrenaline.class) == null) {
						// Use heroFOV (game state) instead of sprite.visible (render state)
						boolean inFOV = Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[mySkeleton.pos];
						if (sprite != null && inFOV){
