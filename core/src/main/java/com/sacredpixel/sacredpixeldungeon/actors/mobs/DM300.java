...
import com.sacredpixel.sacredpixeldungeon.actors.hero.spells.WallOfLight;
import com.sacredpixel.sacredpixeldungeon.effects.FloatingText;
import com.sacredpixel.sacredpixeldungeon.effects.TargetedCell;
...
import com.sacredpixel.sacredpixeldungeon.items.artifacts.LloydsBeacon;
...
public class DM300 extends Mob {
				...
				if (enemy != null && enemy.isAlive() && !canReach){
					...
					if (turnsSinceLastAbility >= MIN_COOLDOWN){
						...
						if (aim.cells.contains(enemy.pos) && !Char.hasProp(enemy, Property.INORGANIC)) {
							...
							if (sprite != null && (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[enemy.pos])) {
	...
	public void die( Object cause ) {
		...
		for (int i = 0; i < shards; i++){
			...
			} while (!Dungeon.level.passable[pos + ofs] && --tries > 0);
			if (tries > 0) {
