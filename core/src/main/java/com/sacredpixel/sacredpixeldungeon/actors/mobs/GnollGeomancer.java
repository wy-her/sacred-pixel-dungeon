...
import com.sacredpixel.sacredpixeldungeon.sprites.GnollGeomancerSprite;
...
public class GnollGeomancer extends Mob {
	...
	protected boolean act() {
		if (sapperSpawns == null){
			// 실제 sapper 개수를 먼저 카운트
			ArrayList<Integer> sapperPosList = new ArrayList<>();
			for (Mob m : Dungeon.level.mobs){
				if (m instanceof GnollSapper){
					sapperPosList.add(((GnollSapper) m).spawnPos);
				}
			}
			sapperSpawns = new int[sapperPosList.size()];
			for (int i = 0; i < sapperPosList.size(); i++){
				sapperSpawns[i] = sapperPosList.get(i);
			}
		}
