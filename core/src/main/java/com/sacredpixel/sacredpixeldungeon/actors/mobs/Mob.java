...
import com.sacredpixel.sacredpixeldungeon.actors.buffs.SoulMark;
...
import com.sacredpixel.sacredpixeldungeon.effects.Surprise;
import com.sacredpixel.sacredpixeldungeon.effects.Wound;
...
public abstract class Mob extends Char {
	...
	protected boolean doAttack( Char enemy ) {
		...
		if (sprite != null && inFOV) {
			// Interrupt hero's auto-movement immediately when a mob begins its
			// attack animation, rather than waiting for the animation callback.
			// On HTML5/TeaVM, the hero can take multiple movement steps during
			// the attack animation because the mob is marked waitingForCallback
			// while the hero keeps being selected by Actor.process().
			if (enemy == Dungeon.hero && Dungeon.hero.damageInterrupt) {
				Dungeon.hero.interrupt();
			}
			//mark that the mob was visible during its attack, so that its next
			//movement will be animated even if heroFOV shifts away in the meantime
