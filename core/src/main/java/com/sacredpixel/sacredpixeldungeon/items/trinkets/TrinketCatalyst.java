...
package com.sacredpixel.sacredpixeldungeon.items.trinkets;
...
public class TrinketCatalyst extends Item {
	...
	public static class Recipe extends com.sacredpixel.sacredpixeldungeon.items.Recipe {
		...
		public Item brew(ArrayList<Item> ingredients) {
			...
			SacredPixelDungeon.scene().addToFront(new WndTrinket(newCata));
			try {
			...
			for (int i = 0; i < NUM_TRINKETS; i++){
				ItemButton btnReward = new ItemButton() {
					...
					protected void onClick() {
						SacredPixelDungeon.scene().addToFront(new RewardWindow(item()));
					}
				};
					...
					protected void onClick() {
						...
						if (cata != null) {
							...
							if (SacredPixelDungeon.scene() instanceof AlchemyScene) {
								((AlchemyScene) SacredPixelDungeon.scene()).craftItem(null, result);
							} else {
