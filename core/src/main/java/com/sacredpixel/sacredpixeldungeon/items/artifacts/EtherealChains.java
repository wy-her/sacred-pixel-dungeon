...
public class EtherealChains extends Artifact {
	...
	private void chainEnemy( Ballistica chain, final Hero hero, final Char enemy ){
		...
		Chains chains1 = new Chains(hero.sprite.center(),
				enemy.sprite.center(),
				Effects.Type.ETHEREAL_CHAIN,
				new Callback() {
			public void call() {
				Pushing pushing = new Pushing(enemy, enemy.pos, pulledPos, new Callback() {
					public void call() {
						enemy.pos = pulledPos;
		...
		chains1.setBlockingVfx();
		hero.sprite.parent.add(chains1);
	}
	...
	private void chainLocation( Ballistica chain, final Hero hero ){
		...
		throwSound();
		Sample.INSTANCE.play( Assets.Sounds.CHAINS );
		Chains chains2 = new Chains(hero.sprite.center(),
				DungeonTilemap.raisedTileCenterToWorld(newHeroPos),
				Effects.Type.ETHEREAL_CHAIN,
				new Callback() {
			public void call() {
				Pushing pushing = new Pushing(hero, hero.pos, newHeroPos, new Callback() {
					public void call() {
						hero.pos = newHeroPos;
		...
		chains2.setBlockingVfx();
		hero.sprite.parent.add(chains2);
	}
