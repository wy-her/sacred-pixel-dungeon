...
import com.sacredpixel.sacredpixeldungeon.items.ItemStatusHandler;
...
import com.sacredpixel.sacredpixeldungeon.items.scrolls.exotic.ScrollOfAntiMagic;
	...
	public static class ScrollToStone extends Recipe {
		...
		public Item brew(ArrayList<Item> ingredients) {
			...
			if (SacredPixelDungeon.scene() instanceof AlchemyScene){
				if (!s.isIdentified()){
					((AlchemyScene) SacredPixelDungeon.scene()).showIdentify(s);
				}
			} else {
				s.identify();
			}
