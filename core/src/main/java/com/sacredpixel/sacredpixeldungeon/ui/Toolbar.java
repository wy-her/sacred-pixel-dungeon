...
import com.sacredpixel.sacredpixeldungeon.windows.WndQuickBag;
...
public class Toolbar extends Component {
	...
	protected void createChildren() {
		...
		for (int i = 0; i < btnQuick.length; i++){
			add( btnQuick[i] = new QuickslotTool(64, 0, 24, 24, i) );
		}
			...
			protected void onClick() {
				if (Dungeon.hero != null && Dungeon.hero.ready && !Actor.anyEnemyAnimating() && !GameScene.cancel()) {
					...
					if (Dungeon.level.getTransition(Dungeon.hero.pos) != null
						...
						Dungeon.hero.next();
					} else if ((Dungeon.level.heaps.get(Dungeon.hero.pos) != null || Dungeon.hero.canSelfTrample())
						&& Dungeon.hero.handle(Dungeon.hero.pos)){
		...
		add(btnSearch = new Tool(44, 0, 20, 26) {
			@Override
			protected void onClick() {
				searchFlashing = false; // Stop flash on click (like journal/hero info buttons)
	...
	protected void layout() {
		...
		//quickslots always numbered left-to-right: startingSlot is leftmost, endingSlot rightmost
		for(int i = startingSlot; i <= endingSlot; i++) {
			if (i == startingSlot){
				btnQuick[i].border(2, 1);
				btnQuick[i].frame(86, 0, 20, 24);
			} else if (i == endingSlot){
				btnQuick[i].border(0, 2);
				btnQuick[i].frame(106, 0, 19, 24);
			} else {
				btnQuick[i].border(0, 1);
		...
		switch(mode){
			case SPLIT:
				...
				//place quickslots left-to-right: endingSlot closest to inventory, startingSlot furthest
				btnQuick[endingSlot].setPos(btnInventory.left() - btnQuick[endingSlot].width(), y + 2);
				...
				if (btnSwap.visible){
					btnSwap.setPos(btnQuick[startingSlot].left() - (btnSwap.width()-2), y+3);
		...
		if (SPDSettings.flipToolbar()) {
			...
			//translate quickslot group to the other side, preserving left-to-right order
			float groupLeft = btnQuick[startingSlot].left();
			float groupRight = btnQuick[endingSlot].right();
			float offset = (right - groupRight) - groupLeft;
			for(int i = startingSlot; i <= endingSlot; i++) {
				btnQuick[i].setPos( btnQuick[i].left() + offset, y+2);
			}

			if (btnSwap.visible){
				btnSwap.setPos( btnSwap.left() + offset, y+3);
			}
