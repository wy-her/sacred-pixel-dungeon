...
public class ScrollPane extends Component implements Focusable {
	...
	public ScrollPane( Component content ) {
		...
		KeyEvent.addKeyListener(keyListener = new Signal.Listener<KeyEvent>() {
			...
			public boolean onSignal(KeyEvent keyEvent) {
				...
				// ZOOM_IN/ZOOM_OUT always work for scrolling (backward compatibility)
				...
				// Arrow key scrolling only when focused
				if (focused) {
					// Up arrow: scroll up
	...
	public void setFocused(boolean focused) {
		...
		if (focused) {
			thumb.am = 1f; // Make thumb fully visible when focused
		} else {
			thumb.am = THUMB_ALPHA;
		}
	}
	...
	@Override
	public void click() {
		// ScrollPane doesn't have a click action, just maintains focus for scrolling
	}
