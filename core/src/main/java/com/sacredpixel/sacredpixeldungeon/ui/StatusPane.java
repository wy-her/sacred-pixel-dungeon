...
import com.sacredpixel.sacredpixeldungeon.effects.CircleArc;
...
public class StatusPane extends Component {
	...
	public StatusPane( boolean large ){
		...
		heroInfo = new Button(){
			...
			protected void onClick () {
				talentBlink = 0; // Stop flash on click (like journal button)
	...
	public void update() {
		...
		int curExp = Dungeon.hero.exp;
		int curMaxExp = Dungeon.hero.maxExp();

		if (large) {
			exp.scale.x = (128 / exp.width) * curExp / curMaxExp;

			hpText.measure();
			hpText.x = hp.x + (128 - hpText.width())/2f;

			if (curExp != lastExp || curMaxExp != lastMaxExp) {
				expText.text(curExp + "/" + curMaxExp);
				expText.measure();
				lastExp = curExp;
			...
			exp.scale.x = ((17 + heroPaneExtraWidth) / exp.width) * curExp / curMaxExp;
			if (curExp != lastExp || curMaxExp != lastMaxExp) {
				expText.text(curExp + "/" + curMaxExp);
				lastExp = curExp;
				lastMaxExp = curMaxExp;
