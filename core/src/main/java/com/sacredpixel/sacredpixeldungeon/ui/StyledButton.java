...
public class StyledButton extends Button {
	...
	private float savedIconRm = 1f, savedIconGm = 1f, savedIconBm = 1f;
	private boolean iconColorSaved = false;
	...
	protected void layout() {
		...
		if (text != null && !text.text().equals("")){
			...
			float textY = y + (height() - text.height()) / 2f;
			if (multiline && text.nLines > 1) textY += 2;
			text.setPos(
	...
	public void setFocused(boolean focused) {
		// Change background brightness (and icon brightness for BLANK chrome type)
		if (focused) {
			bg.brightness(1.3f);
			if (icon != null) {
				// Save current icon color if not already saved
				if (!iconColorSaved) {
					savedIconRm = icon.rm;
					savedIconGm = icon.gm;
					savedIconBm = icon.bm;
				...
				// Apply brightness multiplier to current color
				icon.rm = savedIconRm * 1.5f;
				icon.gm = savedIconGm * 1.5f;
				icon.bm = savedIconBm * 1.5f;
			}
		} else {
			bg.resetColor();
			if (icon != null && iconColorSaved) {
				// Restore saved icon color
				icon.rm = savedIconRm;
				icon.gm = savedIconGm;
				icon.bm = savedIconBm;
				iconColorSaved = false;
			}
		}
	}
	...
	public void restoreFocusState() {
		bg.resetColor();
		// Restore icon color if it was saved
		if (icon != null && iconColorSaved) {
			icon.rm = savedIconRm;
			icon.gm = savedIconGm;
			icon.bm = savedIconBm;
