...
import com.sacredpixel.sacredpixeldungeon.windows.WndInfoTalent;
...
public class TalentButton extends Button {
	...
	protected void onClick() {
		...
		if (parentWindow != null) {
			...
			// Store callback to reactivate on close
			toAdd.setOnHideCallback(new Runnable() {
				@Override
				public void run() {
					if (parentWindow.parent != null) {
						parentWindow.active = true;
						// Restore blocker priority so click-outside-to-close works
		...
		if (SacredPixelDungeon.scene() instanceof GameScene){
			...
			SacredPixelDungeon.scene().addToFront(toAdd);
		}
	}
	...
	public void saveFocusState() {
		// TalentButton doesn't need to save state
	}
