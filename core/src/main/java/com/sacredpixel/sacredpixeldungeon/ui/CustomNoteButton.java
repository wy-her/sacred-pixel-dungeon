...
import com.sacredpixel.sacredpixeldungeon.windows.WndJournalItem;
						...
						protected void onSelect(int index) {
							if (index == 0){
								...
								if (parentWindow instanceof WndJournal || parentWindow == null){
									SacredPixelDungeon.scene().addToFront(new WndJournal());
		...
		GameScene.show(new WndTextInput(promptTitle,
			...
			public void onSelect(boolean positive, String text) {
				if (positive && !text.isEmpty()){
					...
					SacredPixelDungeon.scene().addToFront(wnd);
					SacredPixelDungeon.scene().addToFront(new CustomNoteWindow(note, wnd));
				}
			}
		});
	}
