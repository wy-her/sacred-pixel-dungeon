...
package com.sacredpixel.sacredpixeldungeon.utils;
...
public enum Holiday {
	...
	//Nothing in Jul
	SACREDPD_BIRTHDAY,   //Apr 16th to Apr 23rd                                  (8 days)
	//Nothing in Sept
	...
	public static Holiday getCurrentHoliday(){
		if (cached == null){
			cached = getHolidayForDate(
					DateCompat.getYear(),
					DateCompat.getMonth(),
					DateCompat.getDayOfMonth(),
					DateCompat.getDayOfYear(),
					DateCompat.getDaysInYear()
			);
		}
		return cached;
	}

	public static Holiday getHolidayForDate(int year, int month, int dayOfMonth, int dayOfYear, int daysInYear){

		//Lunar New Year
		if (isLunarNewYear(year, dayOfYear)){
			return LUNAR_NEW_YEAR;
		}

		//April Fools
		if (month == DateCompat.APRIL
				&& dayOfMonth == 1){
			return APRIL_FOOLS;
		}

		//Easter
		if (isEaster(year, dayOfYear, daysInYear == 366)){
			return EASTER;
		}

		//Pride
		if (month == DateCompat.JUNE
				&& dayOfMonth >= 24){
			return PRIDE;
		}
		...
		//Sacred's Birthday
		if (month == DateCompat.APRIL
				&& dayOfMonth >= 16 && dayOfMonth <= 23){
			return SACREDPD_BIRTHDAY;
		}

		//Halloween
		if (month == DateCompat.OCTOBER
				&& dayOfMonth >= 24){
			return HALLOWEEN;
		}

		//Pixel Dungeon's Birthday
		if (month == DateCompat.DECEMBER
				&& dayOfMonth <= 7){
			return PD_BIRTHDAY;
		}
		...
		//New Years
		if ((month == DateCompat.DECEMBER && dayOfMonth >= 27)
				|| (month == DateCompat.JANUARY && dayOfMonth <= 2)){
			return NEW_YEARS;
		}
