...
package com.sacredpixel.sacredpixeldungeon.tiles;
...
public class FogOfWar extends Image {
	...
	*/
	...
	//cached color to avoid redundant Pixmap.setColor calls
	private int lastSetColor = Integer.MIN_VALUE;
	...
	private void updateTexture( boolean[] visible, boolean[] visited, boolean[] mapped ) {
		...
		lastSetColor = Integer.MIN_VALUE;
		...
		//use reupload (glTexSubImage2D) instead of bitmap (glTexImage2D)
		//since the texture is already allocated at the correct size,
		//this avoids GPU-side reallocation on every fog update
		texture.reupload(fog);
	...
	private void setCachedColor( Pixmap fog, int color ){
