...
public class Bundle {
	...
	//set to false on platforms that don't support GZIP (e.g. HTML5/GWT)
	public static boolean compressionSupported = true;

	private static final int GZIP_BUFFER = 1024*4; //4 kb

	public static Bundle read( String json ) throws IOException {
		try {
			Object parsed = new JSONTokener(json).nextValue();
			if (parsed instanceof JSONArray){
				parsed = new JSONObject().put( DEFAULT_KEY, parsed );
			}
			if (!(parsed instanceof JSONObject)){
				throw new JSONException("Malformed JSON Object: " + json);
			}
			return new Bundle( (JSONObject) parsed );
		} catch (Exception e) {
			Game.reportException(e);
	...
	public static Bundle read( InputStream stream ) throws IOException {
		...
		try {
			...
			if (compressionSupported) {
				...
				if( GZIPCompat.isGZIPHeader( header ) ) {
					stream = GZIPCompat.wrapInputForGZIP( stream );
				}
			}
	...
	public static boolean write( Bundle bundle, OutputStream stream, boolean compressed ) {
		try {
			...
			if (compressed && compressionSupported) writer = new BufferedWriter( new OutputStreamWriter( GZIPCompat.wrapOutputForGZIP(stream) ) );
