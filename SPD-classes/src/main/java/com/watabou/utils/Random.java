...
public class Random {
	 ...
	 * This replaces java.util.Random because TeaVM's implementation
	 * does not produce consistent results across different runs with the same seed.
	 */
	private static class DeterministicRandom {
		private long state;

		public DeterministicRandom() {
			// Use system time for unseeded generator
			this.state = System.currentTimeMillis() ^ System.nanoTime();
		}
		...
		// SplitMix64 algorithm - deterministic and platform-independent
		...
		public long nextLong() {
			return nextSplitMix64();
		}

		public int nextInt() {
			return (int) (nextSplitMix64() >>> 32);
		}

		public int nextInt(int bound) {
			if (bound <= 0) return 0;
			// Use rejection sampling to avoid modulo bias
			long r = nextSplitMix64() >>> 1; // Make positive
			return (int) (r % bound);
		}

		public float nextFloat() {
			// Use upper 24 bits for float (standard approach)
			return (nextSplitMix64() >>> 40) / (float)(1 << 24);
		}

		public double nextDouble() {
			// Use upper 53 bits for double (standard approach)
			return (nextSplitMix64() >>> 11) / (double)(1L << 53);
		}
	}
	...
	public static synchronized void resetGenerators(){
		...
		generators.push(new DeterministicRandom());
	}
	...
	public static synchronized void pushGenerator( long seed ){
		long scrambled = scrambleSeed(seed);
		generators.push( new DeterministicRandom( scrambled ) );
	}
	...
	public synchronized static<T> void shuffle( List<?extends T> list){
		// Manual Fisher-Yates shuffle using our deterministic random
		// Use raw types like Collections.shuffle does internally
		List rawList = list;
