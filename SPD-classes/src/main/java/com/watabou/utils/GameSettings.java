...
public class GameSettings {
	...
	//On HTML5, flush() writes the entire preferences map to localStorage which is expensive.
	//We batch changes and only flush every ~1 second to avoid per-frame overhead.
	private static boolean dirty = false;
	private static float dirtyTimer = 0f;
	private static final float FLUSH_INTERVAL = 3f; // seconds
	...
	//Call once per frame (e.g. from Game.render) to flush any pending changes.
	//On HTML5, delays flush to reduce localStorage write frequency.
	public static void flushIfDirty(){
		if (dirty && prefs != null){
			if (DeviceCompat.isHTML5()) {
				dirtyTimer += Game.elapsed;
				if (dirtyTimer >= FLUSH_INTERVAL) {
					prefs.flush();
					dirty = false;
					dirtyTimer = 0f;
				}
	...
	//Force immediate flush (used during save operations)
	...
	public static void put( String key, int value ) {
		...
		dirty = true;
		//On HTML5, defer flush to flushIfDirty() to batch writes.
		//On other platforms flush immediately for safety.
		if (!DeviceCompat.isHTML5()) get().flush();
	}
