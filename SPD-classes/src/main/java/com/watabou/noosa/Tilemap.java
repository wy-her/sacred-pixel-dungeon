...
public class Tilemap extends Visual {
	...
	public synchronized void updateMapCell(int cell){
		...
		//GWT/WebGL glBufferSubData with positioned buffers doesn't work correctly,
		//so force full buffer updates on HTML5
		if (DeviceCompat.isHTML5()) fullUpdate = true;
	}
	...
	public void draw() {
		...
		//calculate visible tile row range to avoid drawing off-screen tiles
		Camera c = camera != null ? camera : Camera.main;
		if (c != null && mapWidth > 0 && cellH > 0) {
			float camTop = c.scroll.y - y;
			float camBottom = camTop + c.height;
			int rowStart = Math.max(0, (int)(camTop / cellH));
			int rowEnd   = Math.min(mapHeight, (int)(camBottom / cellH) + 1);
			int offset   = rowStart * mapWidth;
			int length   = (rowEnd - rowStart) * mapWidth;
			if (length > 0 && offset < size) {
				length = Math.min(length, size - offset);
				script.drawQuadSet( buffer, length, offset );
			}
		} else {
