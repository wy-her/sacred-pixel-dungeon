...
public class Emitter extends Group {
	...
	public void burst( Factory factory, int quantity ) {
		//On HTML5, reduce burst particle count by 50%
		if (DeviceCompat.isHTML5() && quantity > 2) {
	...
	public void update() {
		...
		if (on) {
			...
			//On HTML5, reduce particle emission rate for continuous emitters
			if (DeviceCompat.isHTML5() && quantity == 0 && interval > 0) {
				effectiveInterval = interval * 2f;
			}
			time += Game.elapsed;
			while (time > effectiveInterval) {
			...
			super.update();
		} else if (started && autoKill) {
			//combine the member update loop with the living-particle check
			//to avoid a separate countLiving() scan every frame
