...
public class RenderedText extends Image {
	...
	private static final HashSet<Character> alreadyReported = new HashSet<>();
	private static final int MAX_REPORTED_CHARS = 256;
	private static GlyphLayout sharedLayout;
	...
	private synchronized void measure(){
		...
		if (font != null){
			...
			sharedLayout.setText(font, text);
			GlyphLayout glyphs = sharedLayout;
			...
			for (char c : text.toCharArray()) {
				...
				if (g == null || (g.id != c)){
					...
					//reduces logspam
					if (!alreadyReported.contains(c) && alreadyReported.size() < MAX_REPORTED_CHARS) {
			...
			//Only apply for actual CJK punctuation ranges, not Korean syllables or Latin chars.
			char lastChar = text.charAt(text.length()-1);
			boolean isCJKPunctuation = (lastChar >= '\u3000' && lastChar <= '\u303F')
					|| (lastChar >= '\uFF00' && lastChar <= '\uFFEF');
			BitmapFont.Glyph lastGlyph = font.getData().getGlyph(lastChar);
			if (lastGlyph != null && isCJKPunctuation && lastGlyph.xadvance > lastGlyph.width*1.5f){
				width = glyphs.width - lastGlyph.width + lastGlyph.xadvance;
			} else {
				width = glyphs.width;
			}
			
			//use the font's actual measured capHeight for consistent cross-browser positioning
			height = Math.round(font.getData().capHeight);
	...
	private static class TextRenderBatch implements Batch {
		...
		private static final int MAX_BUFFER_CACHE_SIZE = 16;
		...
		public void draw(Texture texture, float[] spriteVertices, int offset, int count) {
			...
			if (buffers.containsKey(count/20)){
				...
				if (buffers.size() < MAX_BUFFER_CACHE_SIZE) {
