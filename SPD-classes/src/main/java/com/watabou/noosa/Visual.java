...
public class Visual extends Gizmo {
	...
	// DEBUG flag for overlapsScreenPoint tracing
	public static boolean DEBUG_OVERLAPS = false;
	...
	public boolean overlapsScreenPoint( int x, int y ) {
		...
		if (c == null) {
			if (DEBUG_OVERLAPS) {
				System.out.println("[Visual.overlapsScreenPoint] camera is NULL for " + this.getClass().getSimpleName());
			}
			return false;
		}
		if (!c.hitTest(x, y)) {
			if (DEBUG_OVERLAPS) {
				System.out.println("[Visual.overlapsScreenPoint] camera.hitTest FAILED for screenPos(" + x + "," + y + ")" +
					" camera: x=" + c.x + " y=" + c.y + " screenW=" + c.screenWidth() + " screenH=" + c.screenHeight());
			}
			return false;
		}

		PointF p = c.screenToCamera( x, y );
		boolean result = overlapsPoint( p.x, p.y );
		if (DEBUG_OVERLAPS && !result) {
			System.out.println("[Visual.overlapsScreenPoint] overlapsPoint FAILED for cameraPos(" + p.x + "," + p.y + ")" +
				" visual bounds: x=" + this.x + " y=" + this.y + " w=" + (width * scale.x) + " h=" + (height * scale.y));
		}
		return result;
	}
