...
public class PointerEvent {
	...
	// Debug flag to trace listener order
	public static boolean DEBUG_LISTENER_ORDER = false;

	public static void addPointerListener( Signal.Listener<PointerEvent> listener ){
		pointerSignal.add(listener);
		if (DEBUG_LISTENER_ORDER) {
			System.out.println("[PointerEvent.addPointerListener] Added: " + listener.getClass().getSimpleName() +
				" Total listeners: " + pointerSignal.numListeners());
		}
	}
	...
	public static void removePointerListener( Signal.Listener<PointerEvent> listener ){
		...
		if (DEBUG_LISTENER_ORDER) {
			System.out.println("[PointerEvent.removePointerListener] Removed: " + listener.getClass().getSimpleName() +
				" Total listeners: " + pointerSignal.numListeners());
		}
	}
	...
	public static int numPointerListeners(){
		return pointerSignal.numListeners();
	}

	// Debug method to print listener order
	public static void printListenerOrder() {
		System.out.println("[PointerEvent.printListenerOrder] Total listeners: " + pointerSignal.numListeners());
		// Note: We can't access the internal list of Signal, so we'll just print the count
	}
